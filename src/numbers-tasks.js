...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const div = Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(num / div);
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const fullPow = parseInt(`1${'0'.repeat(pow)}`, 10);
  const division = Math.floor(num / fullPow);
  const rem = num % fullPow;
  return rem >= fullPow / 2 ? (division + 1) * fullPow : division * fullPow;
}
